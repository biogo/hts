...
func TestEmpty(t *testing.T) {
	...
	r, err := NewReader(buf, &gzip.Header{})
	if err != nil {
...
func (s *sb) Seek(offset int64, whence int) (ret int64, err error) {
	return
}

// TestSeeker tests that an a Seeker is correctly detected.
func TestSeeker(t *testing.T) {
	...
	if r.s != nil {
		t.Fatal("unexpected seeker")
	}

	sbuf := &sb{new(bytes.Buffer)}
	if err := NewWriter(sbuf).Close(); err != nil {
		t.Fatalf("Writer.Close: %v", err)
	}
	r, err = NewReader(sbuf, &gzip.Header{})
	if err != nil {
		t.Fatalf("NewReader: %v", err)
	}
	if r.s == nil {
		t.Fatal("expected seeker")
	}
}
